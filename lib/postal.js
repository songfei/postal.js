...
_.extend( postal, {
	...
	getSubscribersFor: function getSubscribersFor( options ) {
		...
		_.each( self.subscriptions, function( channel ) {
			_.each( channel, function( subscriber ) {
				_.each( subscriber, function( subList ) {
	...
	publish: function publish( envelope, cb ) {
		...
		var subscriber = envelope.subscriber = envelope.subscriber || _config.DEFAULT_SUBSCRIBER;
		...
		var cacheKey = channel + _config.cacheKeyDelimiter + subscriber + _config.cacheKeyDelimiter + topic;
		...
		if ( !cache ) {
			...
			if ( subscriber === _config.DEFAULT_SUBSCRIBER ) {
				_.each( this.subscriptions[ channel ], function( subscriber ) {
					_.each( subscriber, function( candidates ) {
						_.each( candidates, cacherFn );
					} );
				} );
			} else {
				_.each( this.subscriptions[ channel ][ subscriber ], function( candidates ) {
					_.each( candidates, cacherFn );
				} );
			}
		} else {
	...
	subscribe: function subscribe( options ) {
		...
		var subDef = new SubscriptionDefinition( options.channel || _config.DEFAULT_CHANNEL, options.subscriber || _config.DEFAULT_SUBSCRIBER, options.topic, options.callback );
		...
		var subscriber = subscriptions[ subDef.channel ][ subDef.subscriber ];
		if ( !subscriber ) {
			subscriber = subscriptions[ subDef.channel ][ subDef.subscriber ] = {};
		}
		subs = subscriptions[ subDef.channel ][ subDef.subscriber ][ subDef.topic ];
		if ( !subs ) {
			subs = subscriptions[ subDef.channel ][ subDef.subscriber ][ subDef.topic ] = [];
		}
		...
		_.each( _.keys( cache ), function( cacheKey ) {
			if ( cacheKey.substr( 0, channelLen ) === subDef.channel ) {
				getCacher(
					cacheKey.split( _config.cacheKeyDelimiter )[2],
					cache,
					cacheKey )( subDef );
			}
		} );
	...
	unsubscribe: function unsubscribe() {
		...
		for ( ; unSubIdx < unSubLen; unSubIdx++ ) {
			...
			subscriberSubs = channelSubs && channelSubs[ subDef.subscriber ];
			topicSubs = subscriberSubs && subscriberSubs[ subDef.topic ];
			...
			if ( topicSubs ) {
				...
				if ( topicSubs.length === 0 ) {
					delete subscriberSubs[ subDef.topic ];
					if ( !_.keys( subscriberSubs ).length ) {
						delete channelSubs[ subDef.subscriber ];
					}
